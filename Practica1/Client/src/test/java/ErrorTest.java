...
public class ErrorTest {
    ...
    public void example_test() {
        ...
        try {
            ...
            comUtils.write_int32(id+5); // incorrect id

            assertEquals(8, comUtils.getDataInputStream().readByte());
            assertEquals(id, comUtils.read_int32());
            assertEquals(9, comUtils.getDataInputStream().readByte());
            assertEquals("Invalid session id", comUtils.findString());
    ...
    public void example_test2() {
        ...
        try {
            ...
            comUtils.read_int32();
            comUtils.getDataInputStream().readByte();

            comUtils.getDataOutputStream().writeByte(5);
            comUtils.write_int32(id);
            comUtils.write_string("111");

            assertEquals(8, comUtils.getDataInputStream().readByte());
            assertEquals(id, comUtils.read_int32());
            assertEquals(0, comUtils.getDataInputStream().readByte());
            assertEquals("Unknown move, check the format move format: row-col (from 0 to 2) Example: 0-0", comUtils.findString());
    ...
    public void example_test3() {
        ...
        try {
            ...
            assertEquals("Invalid move, there is already a piece in that position", comUtils.findString());
